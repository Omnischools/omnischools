...
Runs at 02:00 GMT (same window as the nightly snapshot from `backup-dr.html`). Reads from operational, computes school-scoped aggregates, writes to analytics. The inclusion set is every EMIS-registered school that is live on Omnischools — GES/MoE oversight is **statutory**, so there is no data-sharing opt-in to gate on (revised May 2026; both public and private schools are regulated by GES). Coverage stays register-based: a registered school not yet live on Omnischools is excluded from fact computation but still counted in the register.
...
-- (removed) ges_data_sharing_agreements — no per-school consent table: GES/MoE oversight is
-- statutory, so data sharing is mandatory for every registered school (revised May 2026).
