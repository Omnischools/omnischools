...
**Implementer reconciliation (Claude Code note, not Kofi's words):** the DETAIL (severity / support_notes / accommodations) is PER-STUDENT (this §3 ruling), so the grantee card is ONE student card carrying MULTIPLE category pills (primary flagged) + the SINGLE per-student severity/notes/accommodations — NOT per-category detail blocks. Lucy's surface-map assumed per-category detail; this ruling supersedes that part. The rest of Lucy's card anatomy (identity header, pills, notes block, tags, states, privacy strip, responsive) stands.
