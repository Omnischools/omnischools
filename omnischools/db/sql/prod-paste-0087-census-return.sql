-- Omnischools — PROD hand-paste 0087: GOV-8 GES census return (census_return).
--
-- ⚠ RLS IS NOT AUTO-APPLIED ON PROD. `db:policies` only configures LOCAL DEV. This file is the hand-paste
-- that gives the NEW tenant table `census_return` its tenant isolation on PROD. Without it the table has NO
-- boundary and one school's census filing (roll, staffing, facilities snapshot) is readable from every
-- other school's session.
-- Paste this into the Supabase SQL editor on the PROD project AFTER migration 0081 has created the table.
--
...
-- 0081_lame_monster_badoon.sql — this file only applies RLS; run it after the migration, never before.
--
...
--   tenant_tables         +1   (census_return is a real public table with a school_id column)
...
-- Fail-closed guard: the table must exist (created by migration 0081) before RLS can be applied.
DO $$
BEGIN
  IF to_regclass('public.census_return') IS NULL THEN
    RAISE EXCEPTION 'census_return does not exist — run migration 0081 BEFORE this RLS paste';
  END IF;
END
$$;

ALTER TABLE "census_return" ENABLE ROW LEVEL SECURITY;
ALTER TABLE "census_return" FORCE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS tenant_isolation ON "census_return";
CREATE POLICY tenant_isolation ON "census_return"
  FOR ALL TO public
  USING (
...
-- 0087 and safe to run standalone). RESTRICTIVE so it ANDs with (tightens) the permissive tenant_isolation;
...
DROP POLICY IF EXISTS parent_deny ON "census_return";
CREATE POLICY parent_deny ON "census_return" AS RESTRICTIVE FOR ALL TO public
