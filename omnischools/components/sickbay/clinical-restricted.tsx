 ...
 *
 * A server component with NO data access, deliberately: there is nothing to pass. The one prop is
 * the crumb's leaf, so the board can render `Sickbay · Today` (INCR-22c) without a second component.
 * The `Sickbay` link stays in every case — a restricted ADMIN must never be stranded on a wall.
 */
export function ClinicalRestricted({ label = "Visit record" }: { label?: string }) {
  return (
